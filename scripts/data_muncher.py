...
class DataMuncher():
    def __init__(
        ...
        self.my_workers: List[int] = list(range(rank, num_worker_tasks, world_size))
    ...
    def sync_collate(self) -> Tuple[List[torch.Tensor], List[torch.Tensor], List[torch.Tensor], List[torch.Tensor]]:
        ...
        while True:
            for task_id in self.live_workers:
                ...
                thread_save_path = os.path.join("data", "games", self.run_name, f"{group}", f"{task_id}")

                state_path = os.path.join(thread_save_path, f"{self.run_name}_iteration_{self.iter}_states.npy")
                distr_path = os.path.join(thread_save_path, f"{self.run_name}_iteration_{self.iter}_distributions.npy")
                outco_path = os.path.join(thread_save_path, f"{self.run_name}_iteration_{self.iter}_outcomes.npy")
                ...
                if (not task_id in finished_workers and all(os.path.exists(p) for p in [state_path, distr_path, outco_path])):
                    ...
                    tmstps = torch.Tensor([self.iter + 1 if self.use_linear_wgt else 1 for _ in range(states.shape[0])])
            ...
            if start_time is None and len(self.live_workers) > len(finished_workers) > len(self.live_workers) // 2:
                ...
                logger.info("Over half of the workers have finished. Starting timer to kill the rest.")
            ...
            if start_time is not None and time.time() - start_time > self.worker_ttk:
                ...
                logger.info(f"Killing unfinished workers: {len(self.live_workers)} left.")
                break
            ...
            logger.info(f"Spinning on workers to finish... {len(finished_workers)} / {len(self.live_workers)} are complete.")
            time.sleep(30)

        logger.info(f"Total samples for iteration {self.iter}: {sum(s.shape[0] for s in new_states)}")
    ...
    def async_collate(self) -> Tuple[List[torch.Tensor], List[torch.Tensor], List[torch.Tensor], List[torch.Tensor]]:
        ...
        if self.iter == 0:
            ...
            while True:
                ...
                for task_id in range(self.num_worker_tasks):
                    ...
                    states_path = os.path.join(thread_save_path, f"{self.run_name}_iteration_{self.worker_seen[task_id]}_states.npy")
                    distrs_path = os.path.join(thread_save_path, f"{self.run_name}_iteration_{self.worker_seen[task_id]}_distributions.npy")
                    outcos_path = os.path.join(thread_save_path, f"{self.run_name}_iteration_{self.worker_seen[task_id]}_outcomes.npy")

                    if not all(os.path.exists(p) for p in [states_path, distrs_path, outcos_path]):
                        all_workers_have_data = False
                        logger.info(f"Waiting for all workers to have data (missing {task_id})")
                        break
        ...
        for task_id in self.my_workers:
            ...
            while True:
                ...
                if (all(os.path.exists(p) for p in [states_path, distrs_path, outcos_path])):
                    ...
                    # Update the number of iterations we've seen from this worker.
                    self.worker_seen[task_id] += 1

                else:
                    break

        logger.info(f"Total new samples for iteration {self.iter}: {sum(s.shape[0] for s in new_states)}")
    ...
    def get(self):
        ...
        logger.info(f"Before truncating, the total number of games is: {len(self.all_state_tensors)}")
        logger.info(f"Before truncating, the total number of samples is: {sum(s.shape[0] for s in self.all_state_tensors)}")

        assert len(self.all_state_tensors) == len(self.all_distr_tensors) == len(self.all_outco_tensors) == len(self.all_tmstp_tensors)
        ...
        logger.info(f"After truncating, the total number of games is: {len(self.all_state_tensors)}")
        logger.info(f"After truncating, the total number of samples is: {sum(s.shape[0] for s in self.all_state_tensors)}")
        ...
        logger.info(f"Total number of samples used for training: {train_samples}")
        ...
        train_tmstp_tensor = train_tmstp_tensor - torch.min(train_tmstp_tensor) + 1
