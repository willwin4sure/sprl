...
with open("./config/config_controller.json", "r") as f:
    ...
    BOOTSTRAP = config_selfplay["bootstrap"]

assert BOOTSTRAP != "", "Bootstrap must be set in config_selfplay.json for annealing process."

# read the model information off of the saved config file for the bootstrap
with open(f"data/configs/{BOOTSTRAP}_config_controller.json", "r") as f:
    config_controller = json.load(f)
    BOOTSTRAP_MODEL_NUM_BLOCKS = config_controller["modelNumBlocks"]
    BOOTSTRAP_MODEL_NUM_CHANNELS = config_controller["modelNumChannels"]
...
bootstrap_model_kwargs = {
    "num_rows": NUM_ROWS,
    "num_cols": NUM_COLS,
    "action_size": ACTION_SIZE,
    "history_size": HISTORY_SIZE,
    "num_blocks": BOOTSTRAP_MODEL_NUM_BLOCKS,
    "num_channels": BOOTSTRAP_MODEL_NUM_CHANNELS
}
