...
inline int toIndex(int row, int col) {
    return row * SPRL::OTHELLO_SIZE + col;
}

inline bool inBounds(int row, int col) {
    return 0 <= row && row < SPRL::OTHELLO_SIZE && 0 <= col && SPRL::OTHELLO_SIZE;
}

namespace SPRL {

Othello::State Othello::startState() const {
    GameBoard<OTHELLO_SIZE * OTHELLO_SIZE> board{};
    board.fill(-1);
    board[toIndex(3, 3)] = 1;
    board[toIndex(3, 4)] = 0;
    board[toIndex(4, 3)] = 0;
...
const std::vector<int> Othello::captures(const State::Board& board, int row, int col, const Piece piece) const {
    std::vector<int> output{};
    constexpr std::array r_delta{1, 1, 0, -1, -1, -1, 0, 1};
    constexpr std::array c_delta{0, 1, 1, 1, 0, -1, -1, -1};
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        if (inBounds(nextRow, nextCol) && board[toIndex(nextRow, nextCol)] == piece) {
            for (int r = row + r_delta[i]; r != nextRow; r += r_delta[i]) {
                for (int c = col + c_delta[i]; c != nextCol; c += c_delta[i]) {
...
Othello::State Othello::nextState(const State& state, const ActionIdx action) const {
    ...
    // action 64 is a pass
    if (action != OTHELLO_SIZE * OTHELLO_SIZE) {
        // Place the piece there
        newBoard[action] = piece;
        int row = action / OTHELLO_SIZE;
        int col = action % OTHELLO_SIZE;
    ...
    int count0 = 0;
    int count1 = 0;
    for (int i = 0; i < OTHELLO_SIZE * OTHELLO_SIZE; ++i) {
        if (newBoard[i] == 0) {
            count0++;
        }
    ...
    Player winner = -1;
    if (terminal && count0 > count1) winner = 0;
    if (terminal && count1 > count0) winner = 1;
...
Othello::ActionDist Othello::actionMask(const State::Board& board, const Player player) const {
    ...
    for (int row = 0; row < OTHELLO_SIZE; ++row) {
        for (int col = 0; col < OTHELLO_SIZE; ++col) {
    ...
    mask[OTHELLO_SIZE * OTHELLO_SIZE] = canPass ? 1.0f : 0.0f;
...
Othello::State Othello::symmetrizeSingleState(const State& state, Symmetry symmetry) const {
    ...
    case 1: {
        ...
        for (int row = 0; row < OTHELLO_SIZE / 2; ++row) {
            for (int col = 0; col < OTHELLO_SIZE / 2; ++col) {
                Piece temp = board[row * OTHELLO_SIZE + col];
                board[row * OTHELLO_SIZE + col] = board[(OTHELLO_SIZE - 1 - col) * OTHELLO_SIZE + row];
                board[(OTHELLO_SIZE - 1 - col) * OTHELLO_SIZE + row] = board[(OTHELLO_SIZE - 1 - row) * OTHELLO_SIZE + (OTHELLO_SIZE - 1 - col)];
                board[(OTHELLO_SIZE - 1 - row) * OTHELLO_SIZE + (OTHELLO_SIZE - 1 - col)] = board[col * OTHELLO_SIZE + (OTHELLO_SIZE - 1 - row)];
                board[col * OTHELLO_SIZE + (OTHELLO_SIZE - 1 - row)] = temp;
            }
        }
    ...
    case 2: {
        ...
        for (int row = 0; row < OTHELLO_SIZE / 2; ++row) {
            for (int col = 0; col < OTHELLO_SIZE; ++col) {
                std::swap(board[row * OTHELLO_SIZE + col],
                            board[(OTHELLO_SIZE - 1 - row) * OTHELLO_SIZE + (OTHELLO_SIZE - 1 - col)]);
            }
        }
        return State { board, state.getPlayer(), state.getWinner(), state.isTerminal() };
    }
    ...
    case 3: {
        ...
        for (int row = 0; row < OTHELLO_SIZE / 2; ++row) {
            for (int col = 0; col < OTHELLO_SIZE / 2; ++col) {
                Piece temp = board[row * OTHELLO_SIZE + col];
                board[row * OTHELLO_SIZE + col] = board[col * OTHELLO_SIZE + (OTHELLO_SIZE - 1 - row)];
                board[col * OTHELLO_SIZE + (OTHELLO_SIZE - 1 - row)] = board[(OTHELLO_SIZE - 1 - row) * OTHELLO_SIZE + (OTHELLO_SIZE - 1 - col)];
                board[(OTHELLO_SIZE - 1 - row) * OTHELLO_SIZE + (OTHELLO_SIZE - 1 - col)] = board[(OTHELLO_SIZE - 1 - col) * OTHELLO_SIZE + row];
                board[(OTHELLO_SIZE - 1 - col) * OTHELLO_SIZE + row] = temp;
            }
        }
        return State { board, state.getPlayer(), state.getWinner(), state.isTerminal() };
    }
    ...
    case 4: case 5: case 6: case 7: {
        ...
        for (int row = 0; row < OTHELLO_SIZE; ++row) {
            for (int col = 0; col < OTHELLO_SIZE / 2; ++col) {
                std::swap(board[row * OTHELLO_SIZE + col], board[row * OTHELLO_SIZE + OTHELLO_SIZE - 1 - col]);
            }
        }
...
Othello::ActionDist Othello::symmetrizeSingleActionDist(const ActionDist& actionSpace, Symmetry symmetry) const {
    ...
    case 1: {
        ...
        for (int row = 0; row < OTHELLO_SIZE / 2; ++row) {
            for (int col = 0; col < OTHELLO_SIZE / 2; ++col) {
                float temp = newSpace[row * OTHELLO_SIZE + col];
                newSpace[row * OTHELLO_SIZE + col] = newSpace[(OTHELLO_SIZE - 1 - col) * OTHELLO_SIZE + row];
                newSpace[(OTHELLO_SIZE - 1 - col) * OTHELLO_SIZE + row] = newSpace[(OTHELLO_SIZE - 1 - row) * OTHELLO_SIZE + (OTHELLO_SIZE - 1 - col)];
                newSpace[(OTHELLO_SIZE - 1 - row) * OTHELLO_SIZE + (OTHELLO_SIZE - 1 - col)] = newSpace[col * OTHELLO_SIZE + (OTHELLO_SIZE - 1 - row)];
                newSpace[col * OTHELLO_SIZE + (OTHELLO_SIZE - 1 - row)] = temp;
            }
        }
        return newSpace;
    }
    ...
    case 2: {
        ...
        for (int row = 0; row < OTHELLO_SIZE / 2; ++row) {
            for (int col = 0; col < OTHELLO_SIZE; ++col) {
                std::swap(newSpace[row * OTHELLO_SIZE + col],
                            newSpace[(OTHELLO_SIZE - 1 - row) * OTHELLO_SIZE + (OTHELLO_SIZE - 1 - col)]);
            }
        }
        return newSpace;
    }
    ...
    case 3: {
        ...
        for (int row = 0; row < OTHELLO_SIZE / 2; ++row) {
            for (int col = 0; col < OTHELLO_SIZE / 2; ++col) {
                float temp = newSpace[row * OTHELLO_SIZE + col];
                newSpace[row * OTHELLO_SIZE + col] = newSpace[col * OTHELLO_SIZE + (OTHELLO_SIZE - 1 - row)];
                newSpace[col * OTHELLO_SIZE + (OTHELLO_SIZE - 1 - row)] = newSpace[(OTHELLO_SIZE - 1 - row) * OTHELLO_SIZE + (OTHELLO_SIZE - 1 - col)];
                newSpace[(OTHELLO_SIZE - 1 - row) * OTHELLO_SIZE + (OTHELLO_SIZE - 1 - col)] = newSpace[(OTHELLO_SIZE - 1 - col) * OTHELLO_SIZE + row];
                newSpace[(OTHELLO_SIZE - 1 - col) * OTHELLO_SIZE + row] = temp;
            }
        }
        return newSpace;
    }
    ...
    case 4: case 5: case 6: case 7: {
        ...
        for (int row = 0; row < OTHELLO_SIZE; ++row) {
            for (int col = 0; col < OTHELLO_SIZE / 2; ++col) {
                std::swap(newSpace[row * OTHELLO_SIZE + col], newSpace[row * OTHELLO_SIZE + OTHELLO_SIZE - 1 - col]);
            }
        }
...
bool Othello::isTerminal(const State::Board& board) const {
    ...
    return mask0[OTHELLO_SIZE * OTHELLO_SIZE] > 0 && mask1[OTHELLO_SIZE * OTHELLO_SIZE] > 0;
}
