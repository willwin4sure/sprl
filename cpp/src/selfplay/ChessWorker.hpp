#ifndef SPRL_CHESS_WORKER_HPP
#define SPRL_CHESS_WORKER_HPP

#include "../games/ChessNode.hpp"

#include "../networks/INetwork.hpp"
#include "../networks/ChessNetwork.hpp"
...
// Seconds to wait between checking for the model file.
constexpr int MODEL_PATH_WAIT_INTERVAL = 30;

static std::string getTracedModelPath(const std::string& runName, int iteration) {
    if (iteration == -1) return "random";
...
void runChessWorker(
    ...
    INetwork<ChessState, CHESS_ACTION_SIZE>* initialNetwork,
    ...
    INetwork<ChessState, CHESS_ACTION_SIZE>* network;  // Holds the current network.
    ...
    while (true) {
        ...
        ChessNetwork neuralNetwork { modelPath };
        ...
        auto [states, distributions, outcomes] = runIteration<ChessNode, ChessState, CHESS_ACTION_SIZE>(
            iterationOptions,
            treeOptions,
            network,
            nullptr
        ...
        at::Tensor embeddedStates = torch::stack(ChessNetwork::embed(states, flags), 0);
        std::vector<float> embeddedStatesFlattened(embeddedStates.const_data_ptr<float>(), embeddedStates.const_data_ptr<float>() + embeddedStates.numel());

        npy::npy_data_ptr<float> stateData {};
        stateData.data_ptr = embeddedStatesFlattened.data();
        stateData.shape = { static_cast<unsigned long>(states.size()), 17, 8, 8 };
        ...
        distData.shape = { static_cast<unsigned long>(distributions.size()), CHESS_ACTION_SIZE };
